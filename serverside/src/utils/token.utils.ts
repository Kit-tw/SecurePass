...
export const generateAccessToken = (payload: tokenPayload):string =>{
 return jwt.sign(payload,process.env.SECRET_ACCESS_TOKEN as Secret,{expiresIn: '5m',});
}
...
export const VerifyAccessToken = (token : string):tokenPayload =>{
    return jwt.verify(token, process.env.SECRET_ACCESS_TOKEN as Secret) as tokenPayload;
}
