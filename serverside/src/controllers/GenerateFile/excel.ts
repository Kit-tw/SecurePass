...
export const GenerateExcel = async (
  ...
  try {
    ...
    const data = await prismaClient.manageItem.findMany({
      where: { OwnerId: userDB.id },
      orderBy: { id: 'asc' },
    });
    if (!data) {
    ...
    res.setHeader('X-Filename', `${userDB.email.split('@')[0]}_password.xlsx`);
...
const setContent = (sheet: ExcelJs.Worksheet, data: ManageItem[]) => {
  let strRIdx = 2;
  data.map((item) => {
    sheet.getCell(strRIdx, 1).value = item.name;
    sheet.getCell(strRIdx, 2).value = item.URL;
